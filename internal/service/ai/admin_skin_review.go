...
func AdminSkinReviewAnalyze(
	...
	langHead := "**Output locale: Vietnamese (vi).** Warm, lightly witty, never mocking. Write for non-experts (Facebook-group readers): every technical idea in overview/notes must be everyday Vietnamese — nốt đỏ sưng, nốt có đầu trắng, thâm, da bóng, da khô, lỗ chân lông to, da hơi sần… Enum keys (papules/pustules/texture/mild…) OK only in concern/severity/skin_type fields — NEVER paste those English/medical terms into overview, notes, additional_observations, photo_notes, or skin_type_note. Write LONG, information-dense notes — short answers FAIL. Length floors (sentences ending . ! ? …): overview 4–6; skin_type_note exactly 2; each attention note 3–5 (none regions still 3–4 explaining WHY calm); additional_observations 3–5; photo_notes 2–3. Scan forehead→nose→cheeks→chin; mark concern \"none\" ONLY if truly clear. Raised spots/heads/clusters → concern acne|papules|pustules (not redness-only) but describe as nốt đỏ sưng / nốt có đầu trắng / mụn in the note text. Problem notes must cover location detail, count/density, color, shape (sưng/phẳng/đầu trắng), contrast vs nearby zones, and how obvious on photo. Never mention products, brands, mỹ phẩm, or care advice."
	if locale == "en" {
		langHead = "**Output locale: English (en).** Warm, lightly witty, never mocking. Write for non-experts: plain everyday words in overview/notes (red bumps, whiteheads, dark marks, shiny, dry, large pores, uneven surface) — do NOT dump clinical jargon (papules, pustules, erythema, barrier, hyperpigmentation, sebum, morphology) into user-facing text. Enum keys OK only in concern/severity/skin_type fields. Write LONG, information-dense notes — short answers FAIL. Length floors: overview 4–6 sentences; skin_type_note exactly 2; each attention note 3–5 (none regions still 3–4 explaining WHY calm); additional_observations 3–5; photo_notes 2–3. Scan forehead→nose→cheeks→chin; mark concern \"none\" ONLY if truly clear. Raised spots/heads/clusters → concern acne|papules|pustules (not redness-only); note text stays plain English. Problem notes must cover location detail, count/density, color, shape, contrast vs nearby zones, and how obvious on photo. Never mention products, brands, or care advice."
	}
