...
func adminSkinReviewUserText(locale string, compact bool, userQuestion string) string {
	...
	if compact {
		if locale == "en" {
			...
			langHead = "**Output locale: Vietnamese (vi).** Retry rút gọn. Thâm nâu phẳng quanh khóe–cằm / hỏi thâm mép → thâm (CẤM viêm cấp sát mép). Chỉ đỏ sưng/chùm hạt + đau/nổi nhanh → viêm cấp sát mép. Má nốt tròn/mịn không đỏ → mụn ẩn hoặc milia (CẤM papules / mụn thịt). Má sần/gồ không đều không đỏ → texture (CẤM “Nốt đỏ sưng”). Cổ nốt màu da → mụn thịt. Nếp ngang cổ → nếp gấp cổ (CẤM dọa u). Tips khớp case. CẤM nhồi hedge."
		}
	}
