...
func adminSkinReviewUserText(locale string, compact bool, userQuestion string) string {
	...
	if locale == "en" {
		langHead = "**Output locale: English (en).** Best-friend tart voice (I/you), confident on clear photo facts. Morphology: inflammatory / pustules / comedones / peri-oral pigment / acute lip-edge irritation / skin tags / neck creases. Flat brown darkening around mouth corners/chin or user asks about dark marks there → peri-oral pigment — BAN “acute lip-edge irritation”. Only red swollen clustered lip-edge + pain/fast flare → acute lip-edge frame. Neck skin tags → look like skin tags. Horizontal neck creases without raised bumps → neck creases (BAN default skin tags / thyroid scare / “nothing wrong”). Neck/body → region neck|other; photo_notes say neck not face; don't spam 4 face not_visible. Tips match case; no brands/meds/AM-PM/DIY cut."
	}
	if compact {
		if locale == "en" {
