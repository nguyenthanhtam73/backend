...
type OnboardingSkinAnalyzeResponse struct {
	...
	// Morphology verdict from the Go classifier (see service/ai/morphology_classify.go).
	// MorphologyGroup is the group care advice must follow; NeedsMoreInfo means the photo
	// alone cannot separate look-alikes (e.g. milia vs closed comedones) and the user
	// should answer ClarifyQuestions instead of receiving a confident wrong group.
	MorphologyGroup  string   `json:"morphology_group,omitempty"`
	GroupConfidence  string   `json:"group_confidence,omitempty"` // high|medium|low
